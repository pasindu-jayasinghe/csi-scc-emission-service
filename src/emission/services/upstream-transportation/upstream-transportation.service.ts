...
import { UpstreamTransportationEmissionSourceDataMethod } from 'src/emission/enum/upstreamTransportationEmissionSourceDataMethod.enum';
...
import { DTAverageDataMethodDataParameters, BackhaulParameters, DistanceBaseMethodDataParameters, UpstreamTransportationDto, ElectricityParameters, FuelParameters, RefrigerentParameters, SiteSpecificMethodParameters, SpendBaseMethodDataParameters } from './Upstream-transportation.dto';
...
enum UpstreamTransportationFuelBaseType {
  FUEL_DATA = "FUEL_DATA",
...
@Injectable()
export class UpstreamTransportationService implements Iso14064Service, GHGProtocolService {
  constructor(
  ...
  async calculationGHGProtocol(data: UpstreamTransportationDto) {
    let response = new emissionCalResDto();
    switch (data.method) {
      case UpstreamTransportationEmissionSourceDataMethod.FUEL_BASE_METHOD:
        ...
        break;
      case UpstreamTransportationEmissionSourceDataMethod.DISTANCE_BASE_METHOD:
        ...
        break;
      case UpstreamTransportationEmissionSourceDataMethod.SPEND_BASE_METHOD:
        response = await this.calculateSpendBaseData(data.data as SpendBaseMethodDataParameters, data.year, data.month, data.baseData)
        break;
      case UpstreamTransportationEmissionSourceDataMethod.SITE_SPECIFIC_METHOD:
        ...
        break;
      case UpstreamTransportationEmissionSourceDataMethod.AVERAGE_DATA_METHOD:
  ...
  async calculateFuelBaseBackhaul(data: BackhaulParameters, year: number, month: number, baseData: BaseDataDto):Promise<emissionCalResDto>{
    ...
    let fuelUnit = ParamterUnit.uptream_fuel;
  ...
  async calculateFuelBaseElectricity(data: ElectricityParameters, year: number, month: number, baseData: BaseDataDto):Promise<emissionCalResDto>{
    ...
    let elecUnit = ParamterUnit.upstream_electricity_consumption;
  ...
  async calculateDitstanceMethodData(data: DistanceBaseMethodDataParameters, year: number, month: number, baseData: BaseDataDto): Promise<emissionCalResDto> {
    ...
    console.log("hhhh",mass_of_goods_purchased,distance_travelled_in_transport_leg)
  ...
  async calculateSpendBaseData(data: SpendBaseMethodDataParameters, year: number, month: number, baseData: BaseDataDto): Promise<emissionCalResDto> {
    ...
    if(!eEIO_factor){
      let { EEIO } = await this.netZeroFactorService.getNetZeroFactors(year, baseData.countryCode, sourceName.Upstream_Transportation_and_Distribution, [netZeroFactors.EEIO]);
      eEIO_factor = EEIO;
    }
  ...
  async calculateSiteSpecificMethod(data: SiteSpecificMethodParameters, year: number, month: number, baseData: BaseDataDto): Promise<emissionCalResDto> {
    ...
    let volUnit = ParamterUnit.upstream_vol;
    let fuelUnit = ParamterUnit.uptream_fuel;
    let elecUnit = ParamterUnit.upstream_electricity_consumption;
    let refUnit = ParamterUnit.refrigerant_W_RG;
  ...
  async calculateAverageDataMethod(data: DTAverageDataMethodDataParameters, year: number, month: number, baseData: BaseDataDto): Promise<emissionCalResDto> {
    ...
    let volUnit = ParamterUnit.upstream_vol;
    let averageEfUnit = ParamterUnit.upstream_average_ef_unit;
    ...
    if(!storage_facility_ef){
      let { STORAGE_FACILITY_EF } = await this.netZeroFactorService.getNetZeroFactors(year, baseData.countryCode, sourceName.Upstream_Transportation_and_Distribution, [netZeroFactors.STORAGE_FACILITY_EF]);
