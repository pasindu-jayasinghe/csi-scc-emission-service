...
@Injectable()
export class WasteIncinerationService extends TypeOrmCrudService<WasteIncineration>{

  constructor(@InjectRepository(WasteIncineration) repo,) { super(repo) };

  async create(createFuelDto: WasteIncineration) {

    console.log(createFuelDto)
    return await this.repo.save(createFuelDto);
  }

  public async getWasteIncinerationFactors(gasType: string, mswType: string,
    ...
    if (res === -1) {
      console.log("---------------start getWasteIncinerationFactors------------------")
      console.log(countryCode, tier);
      console.log("getWasteIncinerationFactors --- ", res);
      console.log("----------------end getWasteIncinerationFactors-----------------")
    }
    return res;
